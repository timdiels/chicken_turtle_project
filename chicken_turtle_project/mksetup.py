...
def main():
    ...
    # Load project info
    try:
        project = _eval_file(project_root / 'project.py')['project']
    except IOError:
        _critical_exit('Must run from the directory which contains project.py')
    except KeyError:
        _critical_exit('project.py must export a `project` variable (with a dict)')
    ...
    for attr in 'name readme_file description author author_email url license classifiers install_requires keywords'.split():
        if attr not in project:
            _critical_exit('Missing required attribute: project["{}"]'.format(attr))
    ...
    # More checks
    assert 'version' not in project, 'Encountered `version` in `project`. Version should be specified in {}/version.py as `__version__=...`, not in project.py'.format(pkg_root)
    assert 'package_data' not in project, 'Encountered `package_data` in `project`. This is auto-generated, remove it. If your data directories are named `data`, have no `__init__.py` and are part of a package in {}, they will be included by auto generation.'.format(pkg_root)
    assert 'packages' not in project, 'Encountered `packages` in `project`. This is auto-generated by mksetup, remove it.'
    assert 'long_description' not in project, 'Encountered `long_description` in `project`. This is auto-generated by mksetup, remove it.'
    ...
    try:
        ...
        project['version'] = _eval_file(version_path)['__version__'] #TODO check format
    except IOError:
