...
=> Yes, ct-mkdoc implies ct-mkvenv, ct-mkvenv implies ct-mkproject. This means these files cannot be generated by any other program, it's all locked to this workflow, but we lock a lot of workflow anyway. We can still add a --no-mkproject or --no-mkdeps later

---
Point to venv during pre-commit with CT_VENV_DIR instead of symlink.

Setuptools uses absolute paths when installing packages in a venv, this causes
issues when using a symlink some of the time.
---
