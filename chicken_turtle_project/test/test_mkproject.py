...
def create_precommit_project():
    ...
    write_file('README.md', readme1)
    git_('init')
    git_('add', 'requirements.in', 'LICENSE.txt', 'README.md')
...
def test_precommit_invalid(tmpcwd):
    ...
    with assert_process_fails(stderr_matches='(?i)error'):
        git_('commit', '-a', '-m', 'should fail')  # runs the hook
    
# def test_precommit_ignore_unstaged(tmpcwd):
#     '''
#     Unstaged bits should not be included in build
#     '''
#     # unstaged: a file that is used in a test, e.g. simply import from it
#     # ensure the commit fails with error msg
#     
# def test_precommit_include_changes(tmpcwd):
#     '''
#     Changes during precommit must be included in commit
#     
#     Unstaged changes must remain unstaged in files other than files generated by mk-project
#     '''
#     # 
